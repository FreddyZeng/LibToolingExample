'''
Minimal Example integration with Vim.

Before installing make sure one of the following is satisfied:

* Example is in your PATH
* `g:clang_rename_path` in ~/.vimrc points to valid Example executable
* `binary` in Example.py points to valid to Example executable

To install, simply put this into your ~/.vimrc for python2 support

    noremap <leader>cr :pyf <path-to>/Example.py<cr>

For python3 use the following command (note the change from :pyf to :py3f)

    noremap <leader>cr :py3f <path-to>/Example.py<cr>
...
def main():
    ...
    # Get arguments for Example binary.
